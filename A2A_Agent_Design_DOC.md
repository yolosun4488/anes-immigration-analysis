...
┃                                                                               ┃
┃   ① 从 correlationId 恢复 session（discoveryStore / conversationStore）       ┃
...
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
              ...
              ▼                         ▼                         ▼
┏══════════════════════┓   ┏══════════════════════┓   ┏══════════════════════┓
┃  Superagent          ┃   ┃  resource-discovery      ┃   ┃  resource-create V3      ┃
┃  LLM 智能路由         ┃   ┃  资源查询             ┃   ┃  资源创建             ┃
┃  → 直接 reply         ┃   ┃  + discoveryStore    ┃   ┃  + conversationStore ┃
┃  → 或返回 skillId     ┃   ┗══════════════════════┛   ┗══════════════════════┛
┗══════════════════════┛              │                         │
              │                         │                         │
              │              ┌────────┴────────┐                │
...
═══════════════════════════════════════════════════════════════════════════════
  ...
  Client                requestHandler              Superagent           Skill
    ...
    │                        │<───────────────────────│                  │
    │                        │                        │                  │
    │                        │  handler["resource-discovery"](params)         │
    │                        │──────────────────────────────────────────>│
    │                        │                        │                  │ 调下游资源 API
    │                        │                        │                  │ 读写 discoveryStore
    │                        │  result                │                  │
...
|---|---------|------|------|-------------|------|
| 1 | resource-discovery | Fetch Resource | 核心 | `discoverySkill.ts` | 下游资源 API |
| 2 | resource-create | Create Resource V3 | 核心 | `createSkill.ts` | 下游资源 API |
...
const SKILL_HANDLERS = {
  "resource-discovery": discoveryHandler,
  "resource-create":    createHandler,
  "ops-skill":      storeOpsHandler,
  "config-skill":   storeConfigHandler,
  "echo":           echoHandler,
};
...
flowchart TB
    ...
    subgraph L4["Skill 层"]
        ...
        SK1["resource-discovery"]
        SK2["resource-create V3"]
        SK3["ops-skill"]
        SK4["config-skill"]
    end
...
| **resource-discovery** | 资源查询 + 工作流状态机 + 多轮补参 | `discoverySkill.ts` |
| **resource-create V3** | 多轮确认创建 + 全局校验 + 工单 + 状态监控 | `createSkill.ts` |
| **ops-skill** | 容量 / 配额运维 | ops-skill 模块 |
| **config-skill** | 节点配置只读查询 | config-skill 模块 |
| **discoveryStore** | resource-discovery 多轮会话（1h TTL） | `discoverySessionStore.ts` |
| **conversationStore** | resource-create 确认流程（1h TTL） | `dialogSessionStore.ts` |
...
│  discoverySessionStore / dialogSessionStore / 类型 Cache / 元数据 Cache         │
...
| Store | 用途 | TTL | 文件 | 目标层 |
|-------|------|-----|------|--------|
| discoveryStore | resource-discovery 多轮 + 工作流 | 1h | `discoverySessionStore.ts` | → Redis |
| conversationStore | resource-create 多轮确认 | 1h | `dialogSessionStore.ts` | → Redis |
| 下游资源 API Cache | API 响应缓存 | 0（可配） | `discoverySkill.ts` | 可留 In-Memory 或 Redis |
...
用户输入 → small-llm-model (temperature=0.1, 100+ few-shot)
    ...
    ├─ 查资源                → skillId: "resource-discovery"
    ├─ 创建资源              → skillId: "resource-create"
...
1. **Pod 重启丢 Session** — discoveryStore / conversationStore 全在进程内存，滚动发布即中断多轮对话  
...
**答**: correlationId 串联；discoveryStore 存工作流状态；conversationStore 存 resource-create phase。
...
| skillId | Handler |
|---------|---------|
| resource-discovery | `discoverySkill.ts` |
| resource-create | `createSkill.ts` |
...
|----------|----------|----------|------|-----------|
| 多轮 Session 状态 | 高频读 + 高频写 | 分钟~小时，可丢 | **Redis** | discoveryStore、conversationStore |
| 短期对话上下文 | 高频读 + 写 | 30min~1h | **Redis** | resource-discovery 工作流 phase |
...
| 丢了可重建的静态元数据（枚举列表） | 多轮 Session（resource-discovery / resource-create） |
...
- Key 设计：`session:{correlationId}`，`discovery:{correlationId}`，`conv:{correlationId}`
...
| Prompt / Response 日志（metrics） | discoveryStore 工作流状态 |
| 用户操作审计 | 枚举列表热缓存 |
...
**代码改动最小化：** 抽象 `SessionStore` interface，`InMemorySessionStore` → `RedisSessionStore`，discoveryStore / conversationStore 调用方无感知。
...
| `"give me 2 type-A items for store 10001"` | `{ "action": "route", "skillId": "resource-discovery" }` |
| `"create 3 type-B items"` | `{ "action": "route", "skillId": "resource-create" }` |
| `"show store config for 1234"` | `{ "action": "route", "skillId": "config-skill" }` |
...
// 输入: 'Sure! {"action":"route","skillId":"resource-discovery"} is the answer.'
// 输出: '{"action":"route","skillId":"resource-discovery"}'
```
...
LLM 有时把 "create items" 误判为 resource-create，实际用户想 **查** 资源。Regex 做快速 Fallback：
...
| 用户输入 | 匹配？ | 转换 |
|----------|--------|------|
| `"create 2 type-A items"` | ✅ | → `"fetch 2 type-A items"` |
| `"make 3 type-b items"` | ✅ | → `"fetch 3 type-b items"` |
| `"set up a resource item"` | ✅ | → `"fetch a resource item"` |
| `"update items"` | ❌ | 不转换 |
...
| `"item(v3)"` | `"item\\(v3\\)"` | `()` 是分组语法 |
| `"price[USD]"` | `"price\\[USD\\]"` | `[]` 是字符类 |

---

#### 三、完整流程：Item-Discovery 识别链路

```
用户: "give me 3 type-a items, for store 10001"
         │
         ▼
① Superagent (Few-shot + LLM)
   → skillId = "resource-discovery"
         │
         ▼
② paramExtractor (LLM) 或 Regex Fallback
   → count=3, categoryType=?, storeId=10001
         │
         ▼
...
**Q: 为什么不全用 Regex 做路由？**  
A: 用户说法无限（"帮我看看资源"、"show me stuff"、"what items do we have"），Regex 维护成本爆炸；Few-shot 让 LLM 泛化。
